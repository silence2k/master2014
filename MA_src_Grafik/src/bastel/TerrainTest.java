...
public class TerrainTest extends SimpleApplication {
	...
	public static void main(final String[] args) {
		...
		reciver.addDataParser(new ParserSimple(st6d));
		Thread t = new Thread(reciver);
		t.start();
    	
    	
        TerrainTest app = new TerrainTest(st6d);
        app.start();
...
//    	this.getCamera().setLocation(new Vector3f(tmpX, 20, tmpY));
    	...
    	this.getCamera().setLocation(new Vector3f((float)camPos.getX()/scale, 20, -(float)camPos.getY()/scale));
    	
    	Quaternion q = new Quaternion();
    	
    	Vector3f vX = new Vector3f(camPos.getEtaM()[0],camPos.getEtaM()[1],camPos.getEtaM()[2]);
    	Vector3f vY = new Vector3f(camPos.getThetaM()[0],camPos.getThetaM()[1],camPos.getThetaM()[2]);
    	Vector3f vZ = new Vector3f(camPos.getPhiM()[0],camPos.getPhiM()[1],camPos.getPhiM()[2]);
    	
    	System.out.println("CamPos x:" + camPos.getX()+" y: "+camPos.getY());
...
//    	this.getCamera().setAxes(new Quaternion(x, y, z, w));
    ...
    private void updateDelta(){
    	deltaX = 0;
    	deltaY = 0;
    	if(camPos.getX() > grenze){
    		deltaX = deltaDefault;
    	}else if(camPos.getX() < -grenze){
    		deltaX = -deltaDefault;
    	}
    	if(camPos.getY() > grenze){
    		deltaY = -deltaDefault;
    	}else if(camPos.getY() < -grenze){
    		deltaY = deltaDefault;
    	}
